      ...
      filteredBooks.forEach(book => {
        ...
        card.innerHTML = `
	      ...
	      <p class="text-muted">Verkäufer: ${book.seller_name}</p>
              <button class="btn btn-secondary" onclick="show_message_modal('`+JSON.stringify(${Jbook.title})+`','`+JSON.stringify(${book.seller_name})+`','${book.seller}');">Kontakt</button>
	      <button class="btn btn-secondary" onclick="show_info_modal('`+JSON.stringify(${Jbook.title})+`','`+JSON.stringify(${book.seller_name})+`','`+JSON.stringify(${book.author})+`','${book.pages}','${book.language}','${book.date_published}','`+JSON.stringify(${book.book_condition})+`','${book.price}','${book.image_url}');">Mehr Infos</button>
            </div>
          </div>
        `;
